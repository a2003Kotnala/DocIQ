          ...
          <div className="relative z-10">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-4 max-w-4xl text-sm leading-7 text-muted">{result.highlighted_text}</p>
              </div>
              <div className="shrink-0 rounded-2xl border border-[rgba(220,180,110,0.12)] bg-[rgba(255,255,255,0.02)] px-4 py-3 text-right">
            ...
            {result.matched_fields.length ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {result.matched_fields.map((field) => (
                  <span
                    key={`${result.document_id}-${field}`}
                    className="rounded-full border border-[rgba(220,180,110,0.12)] bg-[rgba(255,255,255,0.02)] px-3 py-1 text-xs text-muted"
                  >
                    {field}
                  </span>
                ))}
